...
namespace DearWalletDressMeUp.Model
{
    public static class Pomocna
    {
        ...
        public async static Task<Tuple<bool, string, Korisnik>> ValidacijaLogina(string name,string pass)
        {
            ...
            if (!lista.Any(x => x.Username == name)) return new Tuple<bool, string, Korisnik>(false, "nr", new Korisnik());
            kor=lista.Find(x => x.Username == name);
            if (kor.Sifra != pass) return new Tuple<bool, string, Korisnik>(false, "Username i sifra se ne slazu!", new Korisnik());
